...
export const QuickUserSettings = () => {
  ...
  const handleSaveEmail = () => {
    if (!emailData.newEmail || !emailData.confirmEmail) {
      showSuccess("Fill all email fields");
      return;
    }
    if (emailData.newEmail !== emailData.confirmEmail) {
  ...
  const handleSavePassword = () => {
    ...
    if (passwordData.newPassword.length < 6) {
      showSuccess("Password min 6 chars");
      return;
    }
    if (updateUserProfile && user) {
      updateUserProfile({
        ...
        >
          ...
          <div style={{ marginBottom: "1.2rem", paddingBottom: "1rem", borderBottom: "1px solid #e0d5cc" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
              <h4 style={{ margin: "0", fontSize: "0.95rem", fontWeight: "600", color: "#5e584d" }}>Avatar</h4>
              <span style={{ fontSize: "1.8rem" }}>{userIcon}</span>
            </div>
